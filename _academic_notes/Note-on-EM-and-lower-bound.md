...
$$
P(Z=k;\Theta)=\pi_k\\
P(X|Z=k;\Theta)=\mathcal N(\mu_k,\Sigma_k)
$$
...
$$
\log\mathcal L(\Theta)=\sum_i \log \sum_k q(z=k)\frac{ P(x_i|z=k;\Theta)P(z=k;\Theta)}{q(z=k)}\\
\geq\sum_i \sum_k q(z=k) \log \frac{ P(x_i|z=k;\Theta)P(z=k;\Theta)}{q(z=k)}
$$
...
=\sum_z p(z|x;\Theta) \log \frac{p(x,z;\Theta)}{p(z|x;\Theta)} \\
=\sum_z p(z|x;\Theta) \log p(x;\Theta)\\
=\log p(x;\Theta)
$$
...
$$
J(q^{(t)},\Theta)\\
=\sum_i \sum_k q_i(z=k) (\log P(x_i|z=k;\Theta)+\log P(z=k;\Theta)-\log q_i(z=k))\\
=\sum_i \sum_k q_i(z=k) (\log P(x_i|z=k;\Theta)+\log P(z=k;\Theta))+const 
$$
